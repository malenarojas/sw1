...
router.post('/add', isLoggedIn, async(req, res) => {
    ...
    const newPrj = {
        title,
        joinLink: 'http://144.22.174.111:3000/projects/access/',
...
router.post('/addmember/:id', isLoggedIn, async(req, res) => {
    ...
    if (exists.length <= 0) {
        ...
        await pool.query('INSERT INTO usr_prj set ?', [newUsrPrj]);
        req.flash('success', username + ' añadido');
    } else {
        req.flash('success', username + ' ya es participante');
    }
    res.redirect(req.get('referer'));
});
...
router.get('/open/:id', isLoggedIn, async(req, res) => {
    ...
    const domain = 'http://144.22.174.111:8080/model-c4'
...
router.post('/join', isLoggedIn, async(req, res) => {
    ...
    if (exists.length <= 0) {
        ...
        await pool.query('INSERT INTO usr_prj set ?', [newUsrPrj]);
        req.flash('success', 'Project guardado');
    }
    const userToken = req.user.authToken;
    const accessToken = token;
